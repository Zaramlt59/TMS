<template>
  <div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
      <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">District Overview</h2>
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="text-center">
          <div class="text-3xl font-bold text-blue-600 dark:text-blue-400">{{ data?.schoolsInDistrict?.length || 0 }}</div>
        ...
        <div class="text-center">
          <div class="text-3xl font-bold text-purple-600 dark:text-purple-400">{{ data?.rdBlockDistribution?.length || 0 }}</div>
          <div class="text-sm text-gray-500 dark:text-gray-400">RD Blocks</div>
        </div>
        <div class="text-center">
          <div class="text-3xl font-bold text-orange-600 dark:text-orange-400">{{ data?.performanceMetrics || 0 }}</div>
    ...
    <!-- Schools in District -->
    <div v-if="data?.schoolsInDistrict" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
      <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Schools in Your District</h2>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          <thead class="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">RD Block</th>
            </tr>
          </thead>
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="school in data.schoolsInDistrict" :key="school.school_id">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ school.rd_block }}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <!-- Teacher Statistics -->
    <div v-if="data?.teachersInDistrict" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          ...
          <div class="space-y-2">
            <div v-for="stat in data.teachersInDistrict" :key="stat.gender" class="flex justify-between items-center">
        ...
        <div>
          ...
          <div class="space-y-2">
            <div v-for="stat in data.teachersInDistrict" :key="stat.social_group" class="flex justify-between items-center">
    ...
    <div v-if="data?.rdBlockDistribution" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
      <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">RD Block Distribution</h2>
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        <div
          v-for="block in data.rdBlockDistribution"
          :key="block.rd_block"
          class="flex justify-between items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg"
        >
          <span class="text-sm text-gray-900 dark:text-white">{{ block.rd_block }}</span>
          <span class="text-sm font-medium text-gray-900 dark:text-white">{{ block._count.id }} schools</span>
        </div>
      </div>
    </div>
...
const getTotalTeachers = () => {
  // This would be calculated from the teachersInDistrict data
