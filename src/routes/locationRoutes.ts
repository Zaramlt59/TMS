...
router.post('/rd-blocks', async (req, res) => {
  try {
    ...
    const newRdBlock = await prisma.rd_blocks.create({
      data: {
        name,
...
router.put('/rd-blocks/:id', async (req, res) => {
  try {
    ...
    const updatedRdBlock = await prisma.rd_blocks.update({
...
router.delete('/rd-blocks/:id', async (req, res) => {
  try {
    ...
    const checkResult = await prisma.villages.findMany({
...
router.post('/habitations', async (req, res) => {
  try {
    ...
    const newHabitation = await prisma.villages.create({
      data: {
        name,
...
router.put('/habitations/:id', async (req, res) => {
  try {
    ...
    const updatedHabitation = await prisma.villages.update({
      where: { id: parseInt(id) },
      data: {
        name,
        rd_block_id: parseInt(rd_block_id),
...
router.delete('/habitations/:id', async (req, res) => {
  try {
    ...
    const checkResult = await prisma.teachers.findMany({
      where: { habitation: (await prisma.villages.findUnique({ where: { id: parseInt(id) } }))?.name }
    })
